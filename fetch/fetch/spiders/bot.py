...
class Bot(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for i in range(200, 500):
            base_path = "/html/body/div[2]/div[3]/div/div[4]/div[1]/div/table/tbody[1]/tr[{}]/td[2]//text()"
        ...
        try:
            for idx in range(1, num_countries):
                base_path = "/html/body/div[2]/div[3]/div/div[4]/div[1]/div/table/tbody[1]/tr[{}]/td[{}]//text()"
                country_name = response.xpath(base_path.format(idx, 2)).getall()
                ...
                total_cases = to_integer(format_numerical(response.xpath(base_path.format(idx, 3)).get()))
                new_cases = to_integer(format_numerical(response.xpath(base_path.format(idx, 4)).get()))
                total_deaths = to_integer(format_numerical(response.xpath(base_path.format(idx, 5)).get()))
                new_deaths = to_integer(format_numerical(response.xpath(base_path.format(idx, 6)).get()))
                total_recovered = to_integer(format_numerical(response.xpath(base_path.format(idx, 7)).get()))
                active_cases = to_integer(format_numerical(response.xpath(base_path.format(idx, 8)).get()))
                critical = to_integer(format_numerical(response.xpath(base_path.format(idx, 9)).get()))
                total_cases_by_1M = to_float(format_numerical(response.xpath(base_path.format(idx, 10)).get()))
